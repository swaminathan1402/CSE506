...
void setMap(uint64_t virtual_addr, uint64_t physical_addr){
	...
	if((pml4e+pml4e_index)->p == 0 ){
		...
		// uint64_t *throw_away_page = (uint64_t *)get_free_page();
		memset(some_page, 0, 4096);
		(pml4e + pml4e_index)->page_directory_pointer_base_address = (uint64_t)some_page ;
		...
		if((pdpe + pdpe_index)->p == 0){
			...
			// throw_away_page = (uint64_t *)get_free_page();
			memset(temp_page, 4096, 0);
			(pdpe + pdpe_index)->page_directory_base_address =(uint64_t)temp_page;
                        (pdpe + pdpe_index)->p = 1;
			...
			if((pde + pde_index)->p == 0){
				...
				// throw_away_page = (uint64_t *)get_free_page();
                                (pde + pde_index)->page_table_base_address =(uint64_t) temp_page_1;
                                (pde + pde_index)->p = 1;
                                (pde + pde_index)->rw = 1;
                                (pde + pde_index)->us = 1;
